...
from pysrc.review.train.preprocess import preprocess_part
...
def get_args():
    ...
    parser.add_argument("-mode", dest="mode", default="preprocess", choices=['preprocess', 'merge'],
                        help="mode")
    parser.add_argument("-root2data", dest="root2data", default="~/dataset_with_refs", type=str,
                        help="path todataset with references")
...
def preprocess_paper(paper_id, sentences_df, abstracts_df,
                     figures_df, tables_df, citations_df, reverse_ref_df):
    preprocessed_paper, preprocessed_abstract, preprocessed_gold = [], [], []
    paper = sentences_df[sentences_df['pmid'] == paper_id]['sentence'].values
    abstract = abstracts_df[abstracts_df['pmid'] == paper_id]['abstract'].values[0]
    ...
    captions = []
    captions_fig = figures_df[figures_df['pmid'] == paper_id]['caption'].values
    captions_tables = tables_df[tables_df['pmid'] == paper_id]['caption'].values
    for cap in captions_fig:
        captions.append(parse_sents(cap))
    for cap in captions_tables:
        captions.append(parse_sents(cap))
    captions = standardize(captions)
    ref_df = citations_df[citations_df['ref_pmid'] == paper_id]
    ref_sents_df = pd.merge(ref_df, reverse_ref_df, left_on=['pmid', 'ref_id'], right_on=['pmid', 'ref_id'])
    ref_sents = pd.merge(ref_sents_df, sentences_df, left_on=['pmid', 'sent_type', 'sent_id'],
                         right_on=['pmid', 'type', 'sent_id'])['sentence'].values
    ref_sents = standardize(ref_sents)
    if len(paper) < 30:
        return None

    if len(paper) > 50:
        cheating_ids = greedy_selection(paper, abstract, captions, ref_sents, 50, stop=False)
        cheating_source = [paper[i] for i in cheating_ids]
        paper = cheating_source

        gold_ids = greedy_selection(paper, abstract, captions, ref_sents, 6)
...
def load_data(root2data):
    ...
    return sentences_df, abstracts_df, filelist_df, review_files_df,
    figures_df, reverse_ref_df, citations_df, tables_df

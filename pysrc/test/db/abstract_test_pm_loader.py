...
class AbstractTestPubmedLoader(metaclass=ABCMeta):
    ...
    def get_row(self, article):
        df = self.getPublicationsDataframe()
        return df[df['id'] == str(article.pmid)]

    def test_load_publications_count(self):
        self.assertEqual(len(self.getPublicationsDataframe()), len(self.ids))

    def test_load_publications_check_ids(self):
        self.assertCountEqual(list(self.getPublicationsDataframe()['id'].values), list(map(str, self.ids)))
    ...
    def test_search_noreviews(self):
        ...
        ids_with_review = self.getLoader().search('Article', limit=10, sort=SORT_MOST_RECENT, noreviews=False)
        self.assertListEqual(sorted(ids_with_review), ['1', '10', '2', '3', '4', '5', '6', '7', '8', '9'],
                             'Wrong IDs of papers')

    def test_search_phrase(self):
        self.assertListEqual(['100'], self.getLoader().search('"activity module"', limit=100, sort=SORT_MOST_CITED),
                             'Wrong IDs of papers')
        self.assertListEqual(['100'], self.getLoader().search('"activating modules"', limit=100, sort=SORT_MOST_CITED),
                             'Wrong IDs of papers')
        # Words in all fields
        self.assertListEqual(['100'], self.getLoader().search('dna methylation', limit=100, sort=SORT_MOST_CITED),
                             'Wrong IDs of papers')

        # there is not phrase, but words
        self.assertListEqual([], self.getLoader().search('"active module"', limit=100, sort=SORT_MOST_CITED),
                             'Wrong IDs of papers')
        # plural check
        self.assertListEqual([], self.getLoader().search('"activating module"', limit=100, sort=SORT_MOST_CITED),
                             'Wrong IDs of papers')
        # whole word match
        self.assertListEqual([], self.getLoader().search('positive', limit=100, sort=SORT_MOST_CITED),
                             'Wrong IDs of papers')
        # Words different fields
        self.assertListEqual([], self.getLoader().search('"dna methylation"', limit=100, sort=SORT_MOST_CITED),
                             'Wrong IDs of papers')

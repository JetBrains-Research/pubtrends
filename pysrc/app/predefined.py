...
def _predefined_examples_jobid(examples, semantic_search_available, papers=()):
    ...
    if semantic_search_available:
        semantic_search_predefined = \
            [(t.replace('"', ''),
              PREDEFINED_SEMANTIC_PREFIX + hashlib.md5(t.replace('"', '').encode('utf-8')).hexdigest())
             for t in examples]
    else:
        semantic_search_predefined = []
    ...
    try:
        ...
        for source, predefine_info in PREDEFINED_JOBS.items():
            for query, jobid in predefine_info:
                # Try to load existing data (version check happens here)
                # If a version mismatches, load_or_save_result_data returns None and the cache is cleared
                data = load_or_save_result_data(pubtrends_celery, jobid)
                if data is not None:
                    continue

                # Data is missing or version mismatch - need to recompute
                ready = False

...
class KeyPaperAnalyzer:
    ...
    def load_citation_stats(self):
        ...
        self.df = pd.merge(self.pub_df, self.cit_df, on='pmid', how='outer')
    ...
    def subtopic_analysis(self, sort_components_key='size'):
        ...
        comps = self.df.groupby('comp')['pmid'].apply(list).to_dict()
    ...
    def subtopic_evolution_analysis(self, step=2, min_papers=0):
        ...
        for i, year in enumerate(year_range):
            ...
            self.logger.info(f'{year}: graph contains {len(CG[year].nodes)} nodes, {len(CG[year].edges)} edges')

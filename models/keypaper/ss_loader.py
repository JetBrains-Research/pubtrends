...
class SemanticScholarLoader(Loader):
    ...
    def search(self, *terms, current=0, task=None):
        ...
        self.values = ', '.join(['({0}, \'{1}\')'.format(i, j) for (i, j) in zip(self.crc32ids, self.ssids)])
    ...
    def load_publications(self, current=0, task=None):
        ...
        self.logger.info('Created table for request with index.', current=current, task=task)
    ...
    def load_citation_stats(self, filter_citations=True, current=0, task=None):
        ...
        self.logger.info(f"Loaded citation stats for {len(self.cit_df)} of {len(self.ssids)} articles.\n" +
